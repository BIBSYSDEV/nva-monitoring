...
public class FakeApiGatewayClient implements ApiGatewayClient {
  ...
  @Override
  public GetRestApisResponse getRestApis(GetRestApisRequest getRestApisRequest)
      throws AwsServiceException, SdkClientException {
    return GetRestApisResponse.builder().items(randomApis()).build();
  }

  @Override
  public GetRestApisIterable getRestApisPaginator() throws AwsServiceException, SdkClientException {
  ...
  private List<RestApi> randomApis() {
    return List.of(RestApi.builder().name(API_1).build(), RestApi.builder().name(API_2).build());
  }
}
